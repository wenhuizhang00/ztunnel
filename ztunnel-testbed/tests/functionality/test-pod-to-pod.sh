...
echo_pod_ip=$(kubectl get pods -n sample-apps -l app=http-echo -o jsonpath='{.items[0].status.podIP}' 2>/dev/null)
[[ -n "$echo_pod_ip" ]] || fail "No http-echo pod IP found"

# Request to pod IP:8080 (http-echo listens on 8080)
result=$(kubectl exec -n sample-apps "$client_pod" -c curl -- curl -s -m 5 "http://${echo_pod_ip}:8080/" 2>/dev/null || echo "FAIL")
