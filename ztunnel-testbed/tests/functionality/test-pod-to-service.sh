...
# Use Service DNS: http-echo.grimlock.svc.cluster.local (or short http-echo)
result=$(kubectl exec -n grimlock "$client_pod" -c curl -- curl -s -m 5 "http://http-echo.grimlock.svc.cluster.local:80/" 2>/dev/null || echo "FAIL")
